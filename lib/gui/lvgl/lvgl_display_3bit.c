		...
		const lv_area_t *area, lv_color_t *color_p)
{
	...
	/* INFO: Formula hardcoded for LPM0XX */
...
{
	/* x and y should be absolute co-ordinates and vdb should 100%
	 * For this _lv_blend_fill is modified to provide absolute co-ordinates
	 */
	/* INFO: Calculation of address is hardcoded for LPM0XX */
	uint8_t *buf_xy = (uint8_t *)(buf + ((x >> 1) + 1) +  (y * ((buf_w >> 1) + 2)));
	uint8_t color_3bit = 0;

	/* INFO: Add thresholding if absolutly required. it adds about
	 * 6ms to draw time.
	 */
		...
		lv_area_t *area)
{
	...
	if (cap.screen_info & SCREEN_INFO_ALIGNMENT_RESTRICTED) {
		if (cap.x_alignment != 0) {
			/* INFO: Hardcoded for LPM0XX */
			area->x1 = 0;
			area->x2 = 175;
		}

		if (cap.y_alignment != 0) {
